...
int NoximProcessingElement::get_slice(NoximCoord src_coord, NoximCoord dest_coord){
	...
	else return 4;    // Mem Controller chooses slice 4
...
void NoximProcessingElement::push_packet(){
	...
	if (true) {
		...
		int mem_c = packet.dst_id;  // This is a hack to find out the memory controller number which is used to reserve the token. should change it
...
NoximFlit NoximProcessingElement::nextFlit(int slice)
{
	...
	if(!is_mc(id2Coord(local_id))){
		int mem_c = packet.dst_id; // Another bloody hack.
